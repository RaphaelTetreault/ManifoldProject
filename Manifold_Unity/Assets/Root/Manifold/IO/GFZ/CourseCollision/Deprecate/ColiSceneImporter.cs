        ...
        IImportable
    {
        ...
        public void TestTransformHeirarchy(ColiSceneSobj sceneSobj)
        {
            ...
            foreach (var trackSegment in scene.rootTrackSegments)
            {
                ...
                var trackCurves = trackSegment.trackCurves;
                float3 timeScale = new float3(
                    GetCurveTime(trackCurves.unityCurves[0]),
                    GetCurveTime(trackCurves.unityCurves[1]),
                    GetCurveTime(trackCurves.unityCurves[2]));
                float3 timeRotation = new float3(
                    GetCurveTime(trackCurves.unityCurves[3]),
                    GetCurveTime(trackCurves.unityCurves[4]),
                    GetCurveTime(trackCurves.unityCurves[5]));
                float3 timePosition = new float3(
                    GetCurveTime(trackCurves.unityCurves[6]),
                    GetCurveTime(trackCurves.unityCurves[7]),
                    GetCurveTime(trackCurves.unityCurves[8]));

                for (float t = 0f; t < 1f; t += increment)
                {
                    float3 scale = new float3(
                        trackCurves.unityCurves[0].EvaluateDefault(t * timeScale.x, 1),
                        trackCurves.unityCurves[1].EvaluateDefault(t * timeScale.y, 1),
                        trackCurves.unityCurves[2].EvaluateDefault(t * timeScale.z, 1));
                    float3 rotation = new float3(
                        trackCurves.unityCurves[3].EvaluateDefault(t * timeRotation.x, 0),
                        trackCurves.unityCurves[4].EvaluateDefault(t * timeRotation.y, 0),
                        trackCurves.unityCurves[5].EvaluateDefault(t * timeRotation.z, 0));
                    float3 position = new float3(
                        trackCurves.unityCurves[6].EvaluateDefault(t * timePosition.x, 0),
                        trackCurves.unityCurves[7].EvaluateDefault(t * timePosition.y, 0),
                        trackCurves.unityCurves[8].EvaluateDefault(t * timePosition.z, 0));
                    ...
                    cube.transform.position = position + trackSegment.localPosition;
                    cube.transform.rotation = Quaternion.Euler(rotation + trackSegment.localRotation);// math.mul(quaternion.EulerXYZ(rotation), quaternion.EulerXYZ(tt.localRotation));
                    cube.transform.localScale = scale * trackSegment.localScale;
                }
            }
        }
